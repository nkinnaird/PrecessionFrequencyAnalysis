...
bool saveAllFits = false; // turn to false before fitting many files
...
TF1* fitAndFillHists(int fileNum, int sampleNum, TH1D* wiggle, TF1* fiveParamFit, double fitStart, double fitEnd, TH1F* RPull_hist, TH1F* chi2_ndf_hist, TH1F* pValues_hist, TNtuple* fitParamsTree)
{
    ...
    if((fileNum == 0 && sampleNum == 0) || saveAllFits){
...
int fitAnalyzerHists(string filePath)
{
  ...
  TFile* outputFile = new TFile("analyzerFits.root","RECREATE");

  int totalFiles = fileVector.size();
  int totalSamples = (*(TVectorD*) firstFile->Get("Samples"))[0];
  // int totalSamples = 1;
  int totalSeeds = (*(TVectorD*) firstFile->Get("Seeds"))[0];

  cout << "Number of files: " << totalFiles << endl;
  cout << "Number of samples per file: " << totalSamples << endl;
  cout << "Number of seeds per file: " << totalSeeds << endl;

  firstFile->Close();

  if(totalFiles > 1 && saveAllFits){
    cout << "Trying to save all fits while feeding in multiple files." << endl;
    exit(0);
  }
...
/////////////////////////////////////////////////////////////////////////////////////
  ...
  double nick_fit_start = 49982.0;
  double nick_fit_end =  650064.4;

  // double david_fit_start = 30287.6;
  double david_fit_start = 49982.0;
...
{
  cout << endl << "File number: " << fileNum << "/" << totalFiles << endl;
  ...
  for (int sampleNum = 0; sampleNum < totalSamples; ++sampleNum)
  {
    ...
    fitAndFillHists(fileNum, sampleNum, nick_T_wiggle, fiveParamFit, nick_fit_start, nick_fit_end, nick_T_RPull, nick_T_chi2_ndf, nick_T_Pvalues, nick_T_fitParams);
    delete nick_T_wiggle;
      ...
      for (int seedNum = 0; seedNum < totalSeeds; ++seedNum)
      {
        ...
        if((seedNum == 0 && fileNum == 0 && sampleNum == 0) || saveAllFits){
        ...
        nick_R_R_err_total += ratioFit->GetParError(1);
        nick_R_phi_total += ratioFit->GetParameter(2);
        nick_R_phi_err_total += ratioFit->GetParError(2);
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    fitAndFillHists(fileNum, sampleNum, david_T_wiggle, fiveParamFit, david_fit_start, david_fit_end, david_T_RPull, david_T_chi2_ndf, david_T_Pvalues, david_T_fitParams);
    ...
    fitAndFillHists(fileNum, sampleNum, david_A_wiggle, fiveParamFit, david_fit_start, david_fit_end, david_A_RPull, david_A_chi2_ndf, david_A_Pvalues, david_A_fitParams);
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    fitAndFillHists(fileNum, sampleNum, aaron_T_wiggle, fiveParamFit, aaron_fit_start, aaron_fit_end, aaron_T_RPull, aaron_T_chi2_ndf, aaron_T_Pvalues, aaron_T_fitParams);
    ...
    fitAndFillHists(fileNum, sampleNum, aaron_A_wiggle, fiveParamFit, aaron_fit_start, aaron_fit_end, aaron_A_RPull, aaron_A_chi2_ndf, aaron_A_Pvalues, aaron_A_fitParams);
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    fitAndFillHists(fileNum, sampleNum, matteo_T_wiggle, fiveParamFit, matteo_fit_start, matteo_fit_end, matteo_T_RPull, matteo_T_chi2_ndf, matteo_T_Pvalues, matteo_T_fitParams);
    ...
    fitAndFillHists(fileNum, sampleNum, matteo_A_wiggle, fiveParamFit, matteo_fit_start, matteo_fit_end, matteo_A_RPull, matteo_A_chi2_ndf, matteo_A_Pvalues, matteo_A_fitParams);
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    fitAndFillHists(fileNum, sampleNum, bingzhi_T_wiggle, fiveParamFit, bingzhi_fit_start, bingzhi_fit_end, bingzhi_T_RPull, bingzhi_T_chi2_ndf, bingzhi_T_Pvalues, bingzhi_T_fitParams);
    ...
    fitAndFillHists(fileNum, sampleNum, bingzhi_A_wiggle, fiveParamFit, bingzhi_fit_start, bingzhi_fit_end, bingzhi_A_RPull, bingzhi_A_chi2_ndf, bingzhi_A_Pvalues, bingzhi_A_fitParams);
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    fitAndFillHists(fileNum, sampleNum, tim_Q_wiggle, fiveParamFit, tim_fit_start, tim_fit_end, tim_Q_RPull, tim_Q_chi2_ndf, tim_Q_Pvalues, tim_Q_fitParams);
    delete tim_Q_wiggle;
