...
int MethodCorrelationsNoRand(std::string filePath)
{
...
/////////////////////////////////////////////////////////////////////////////////////
  ...
  TFile* outputFile = new TFile("methodFitCorrelationsNoRand.root","RECREATE");
...
/////////////////////////////////////////////////////////////////////////////////////
 ...
 // construct correlation matrices for each individual parameter among the different methods
  ...
  vector<string> methodTypes = {"T", "A", "Q"}; // make sure the pointers down below correspond to the correct labels
  ...
  for (uint firstPar = 0; firstPar < parameter_strings.size(); ++firstPar)
  {
    for (uint secondPar = firstPar; secondPar < parameter_strings.size(); ++secondPar)
    {
      ...
      // tried to put everything below here in a separate method to make things easier to code, but had trouble dealing with 2D and 3D arrays when attempting to pass them to another method
        ...
        vector<double*> typePointersA = {Tmethod_Pars[firstPar], Amethod_Pars[firstPar], Qmethod_Pars[firstPar]};
        vector<double*> typePointersB = {Tmethod_Pars[secondPar], Amethod_Pars[secondPar], Qmethod_Pars[secondPar]};
