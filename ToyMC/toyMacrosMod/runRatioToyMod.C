...
// generate histograms for many iterations
   ...
   size_t slash = toyFitConditions.directoryString.find_last_of("/");
   string iterPart = toyFitConditions.directoryString.substr(0, slash);
   string subIterPart = toyFitConditions.directoryString.substr(slash+1);


  // auto toyMCDir = inDir->mkdir(("ToyMC" + toyFitConditions.directoryString).c_str());
  auto toyMCDir = inDir->mkdir(subIterPart.c_str());
...
/*
  ...
  firstFile->Close();
*/
double toyFitEnd = toyFitConditions.fitEndTime;
/////////////////////////////////////////////////////////////////////////////////////
  ...
  RatioCBOFit ratioCBOFitClass(toyFitConditions.fitStartTime, toyFitEnd, myBlinder); // this toy mc code needs a little work/checking to make sure it's set up for cbo fitting
  ratioCBOFitClass.changingCBOFrequency(true);
  ratioCBOFitClass.fitAandPhiCBO(false, false);
...
for (int fileNum = 0; fileNum < int(fileVector.size()); ++fileNum)
{
  ...
  TFile* inputFile = TFile::Open(fileVector.at(fileNum).c_str());
   ...
   TVectorD* histSavedParameters = (TVectorD*) inputFile->Get(("topDir/" + iterPart + "/SavedParameters/parameterStore").c_str());
...
/*
      TH1F* toyFiveParamHist = (TH1F*) ((TH1F*) inputFile->Get(("topDir/" + toyFitConditions.directoryString + "/Toy_5_Param_Hist").c_str()))->Clone();
            ...
            // if(fileNum == 0){
              ...
              double cbo_freq = residualPlotsToyClass.findPeaks(fiveFitFFT);
...
/////////////////////////////////////////////////////////////////////////////////////
            ...
            // get multiplier hists - only works for five iter stuff at the moment

            TH1F* errorMultHist = 0;

            int iterNum = iterPart.at(iterPart.size()-1) - '0';

            // if(iterNum == 0); // do nothing
            // else if(iterNum == 1) errorMultHist = (TH1F*) ((TH1F*) inputFile->Get(("topDir/" + iterPart + "/PileupPlots/errorCompareDir/errorMultiplierByBin").c_str()))->Clone();
            // else if(iterNum == 2) errorMultHist = (TH1F*) ((TH1F*) inputFile->Get(("topDir/" + iterPart + "/PileupPlots/errorCompareDir/errorModHist").c_str()))->Clone();
            errorMultHist = (TH1F*) ((TH1F*) inputFile->Get(("topDir/" + iterPart + "/PileupPlots/errorCompareDir/errorMultiplierByBin").c_str()))->Clone();
...
/////////////////////////////////////////////////////////////////////////////////////
            ...
            // apply modified fit errors here

            // if(iterNum == 0); // do nothing
            // else 
              modifyGraphErrors(toyRatioGraph, errorMultHist);

/////////////////////////////////////////////////////////////////////////////////////

            ratioFitToyClass.ratioFitMethod(toyRatioGraph);

            auto toyRatioFit = toyRatioGraph->GetFunction("ratioFitIntegral");
...
/*
            ...
            ratioCBOFitClass.setCBOFreq(double cbo_freq);
...
int runRatioToyMod(std::string filePath)
{
...
/////////////////////////////////////////////////////////////////////////////////////
  ...
  if(extension == ".root") // if I'm running over a single root file, which possibly has multiple iterations
  {
    ...
    TVectorD* itersDouble = ((TVectorD*) outputFile->Get((fileName + "/Iters").c_str()));
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int iter = 0; iter < totalIters; ++iter)
    {
      ...
      auto toyMCIterDir = topDir->mkdir(("ToyMC" + dirString).c_str());

      for (int subIter = 0; subIter < subIters; ++subIter)
      {
        string secondDirString = dirString + "/SubIter" + to_string(subIter);

        FitCondStruct toyFitConditions(secondDirString, toyFitStart, toyFitEnd, 0, 0);
        toyMC(filePath, toyMCIterDir, toyFitConditions);
      }
    }
  ...
  {
    cout << "This doesn't work with the current setup of this file (without more work) - exiting." << endl;
