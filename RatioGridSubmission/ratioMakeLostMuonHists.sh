...
# Histograms are made with LostMuonHistograms.C, but the macro can be switched if desired.
...
# Use as: . ratioMakeLostMuonHists.sh inputFileList.txt
...
TARNAME="LostMuonHistsTar.tar.gz"

read -p "Update tar file? (y/n) " yn
if [ "$yn" == "y" ]; then
  tar cfz $TARNAME ../HistMaking/LostMuonHistograms.C ../ratioMacroHeaders
fi

#Make sure tar file exists
if [ ! -f ${TARNAME} ]; then
  echo "Tar ball \"${TARNAME}\" is required (and must contain LostMuonHistograms.C).  This was not found."
  return
fi
...
for file in `cat "$inputFile"`; do
  ...
  uniqueID=${uniqueID%.*}
  if [ -f ${pnfsOutDir}/lostMuons_hists_${uniqueID}.root ]; then 
    echo "ERROR: ${pnfsOutDir}/lostMuons_hists_${uniqueID}.root already exists and would be overwritten."
    return
  fi
done
...
#Run root macro
root -l -b -q HistMaking/LostMuonHistograms.C+\(\"\${inFile}\"\)
...
#Copy output back
if [ -f lostMuons_hists_\${uniqueID}.root ]; then 
  ifdh mv lostMuons_hists_\${uniqueID}.root ${pnfsOutDir}/lostMuons_hists_\${uniqueID}.root
else 
  echo "lostMuons_hists_\${uniqueID}.root does not exist.  ls reads:"
  ls
